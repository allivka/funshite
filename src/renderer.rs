...
impl Renderer {
    ...
    pub fn calculate_focal_length(&mut self, canvas: &Canvas) {
        self.focal_length = (canvas.width as f32) / (2.0 * (self.fov_degrees.to_radians() / 2.0).tan());
    }

    pub fn init(mut self, canvas: &Canvas) -> Self {
    ...
    pub fn render(&self, canvas: &mut Canvas, camera: &Viewer) {
        ...
        // for (v1_idx, v2_idx) in self.index_pair_buffer.iter() {
        //     let mut v1 = self.objects[
        //         v1_idx.obj_idx].vertices[self.objects[v1_idx.obj_idx].faces[v1_idx.face_idx][v1_idx.self_idx].vertex
        //         ];
        //     let mut v2 = self.objects[
        ...
        //             v3d_buff = obj.vertices[vertex.vertex];
        //
        ...
        //
        //             v2i_vec.push(Vec2i(
        //                 (canvas.width as f64 / 2.0 + (v3d_buff.0 * self.focal_length as f64) / -v3d_buff.2) as isize,
        //                 (canvas.height as f64 / 2.0 - (v3d_buff.1 * self.focal_length as f64) / -v3d_buff.2) as isize,
        //             ));
        //         }
        //
